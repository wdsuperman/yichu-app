...
class Home extends Component {
  render() {
    ...
    const postList = posts.map((t,index) => index === 1 ? <Link to='lovemy' style={{width:'90%',margin:'0 auto',display:'block',color:'#000'}} key={t.id}>
    ...
    <hr/>
    </Link> : <div style={{width:'90%',margin:'0 auto'}} key={t.id}>
    <div className='top' style={{borderLeft:'1px solid red',display:'flex',justifyContent:'space-between',alignItems:'center',marginTop:'25px'}}>
      <span style={{textIndent:'10px'}}>
        {t.left}
      </span>
